...
\begin{lstlisting}[language=Bash, caption={Ausführung des Dummy Scripts}]
...
\begin{lstlisting}[language=Python, caption={Zugriff auf tbody Elemente}]
...
\begin{lstlisting}[language=Python, caption={Umformung des tbody-Elements zu lesbaren Mensaangeboten}]
for i in range(10):
...
with open(f'{folder_location}mensadata.json', 'w+') as f:
...
\begin{lstlisting}[language=Python, caption={HTML-Inhalt auslesen}]
...
\begin{lstlisting}[language=Python, caption={Ausschneiden von Bilddateien}]
...
\begin{lstlisting}[language=Python, caption={Zusammenfügen von Bilddateien}]
def mergeImgs(imgs):
...
\begin{lstlisting}[language=Bash, caption={Cronjob - getMensaPlan.py}]
...
\begin{lstlisting}[language=Java, caption={NodeJS - Erstellung der Mensa-API}]
...
Dieses API-System wurde genau für solch einen Anwendungsfall, wie wir ihn erstellen wollen, entwickelt. Für gewöhnlich werden 3D-Navigationen in großen Gebäudekomplexen und Kaufhäusern verwendet, um sie dann auf digitalen Endgeräten wie Smartphones oder Digitalterminals für Besucher zur Orientierung zu Verfügung zustellen (Abb. \ref{fig:kaufhausbilder}).\\
...
\begin{tabular}{| l | p{9.45cm} | c| } \hline
    ...
    bzw. schnell zu erreichenden Weg an. Obwohl es nur zwei verschiedene Zustände für die Art eines Videos gibt, haben wir (\verb|create_metadata.py|) uns hierbei bewusst für einen String und nicht für einen booleschen Wert entschieden. Der Grund dafür ist, dass sich bei der Erstellung des Skriptes gezeigt hat, dass wir so keine Datentypen bei der Strukturierung eines Requests umwandeln müssen. Außerdem haben wir so die Möglichkeit, über genügend Agilität zu verfügen, sollten wir dieses Skript zukünftig für ein Projekt verwenden, welches mehr als zwei Modi besitzt. & String                                                                                                                                                                                                                                                                                                        \\[0.5ex]
    \hline
